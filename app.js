(function () {
  ...
  function renderDirectorsPage() {
    ...
    for (var i = 0; i < langData.directors.length; i++) {
      var director = langData.directors[i];
  ...
  /**
   * Remplit le banner mosaïque avec des photos aléatoires (sans répétition récente).
   */
  function initMosaicBanner() {
    var banner = document.getElementById("mosaic-banner");
    if (!banner) return;

    // Récupérer la mémoire des photos déjà affichées
    var memory = [];
    try {
    ...
    // Construire un tableau [1..100] mélangé
    var pool = [];
    ...
    // Trier : photos pas encore vues en premier
    var unseen = [];
    var seen   = [];
    ...
    // Charger exactement 6 photos disponibles (en séquence)
    var loaded = [];
    var cursor = 0;
    var TARGET = 6;

    function loadNext() {
      // Arrêter si on a atteint la cible ou épuisé le pool
      if (loaded.length >= TARGET || cursor >= ordered.length) return;
      ...
      img.src = basePath + "/images/photo (" + num + ").jpg";
      img.alt = "";

      img.onload = function () {
        // Image chargée : l'ajouter au banner et mettre à jour la mémoire
        ...
        // Charger la suivante seulement si on n'a pas encore atteint TARGET
        loadNext();
      };

      img.onerror = function () {
        // Photo manquante : essayer la suivante sans l'ajouter
        loadNext();
      };
    }

    // Lancer le chargement de la première image (les suivantes se chaînent via onload)
    loadNext();
  }
  ...
  function init() {
    ...
    fetch(basePath + "/data.json")
      ...
      .catch(function () {
        // Correction #1 : message d'erreur si data.json est inaccessible
