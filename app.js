(function () {
  ...
  function bindTierFilter() {
    ...
    function applyFilter(tier) {
      ...
      // Mettre à jour l'état actif des boutons tier-filter
      buttons.forEach(function (b) { b.classList.remove("active"); });
      buttons.forEach(function (b) {
        if (b.dataset.tier === tier) b.classList.add("active");
      });

      // Mettre à jour l'état actif des boutons légende
      legendBtns.forEach(function (b) { b.classList.remove("active"); });
      ...
      // Masquer les groupes d'années sans films visibles
      ...
      // Masquer les groupes de réalisateurs sans occurrences visibles
      document.querySelectorAll(".director-group").forEach(function (group) {
        if (tier === "all") {
          group.style.display = "";
          return;
        }
  ...
  function renderDirectorsPage() {
    ...
    for (var i = 0; i < sortedDirectors.length; i++) {
      ...
      if (director.name === "Straub/Huillet") {
        portraitHtml =
          '<img class="director-portrait director-portrait--duo" src="' + basePath + '/portraits/portrait-Straub.jpg" alt="" onerror="this.parentNode.removeChild(this)">' +
          '<img class="director-portrait director-portrait--duo" src="' + basePath + '/portraits/portrait-Huillet.jpg" alt="" onerror="this.parentNode.removeChild(this)">';
      } else {
        var portraitPath = basePath + "/portraits/portrait-" + director.name + ".jpg";
        portraitHtml = '<img class="director-portrait" src="' + portraitPath + '" alt="" onerror="this.parentNode.removeChild(this)">';
      }
  ...
  function renderLegend(isDirectorsPage) {
    ...
    for (var i = 0; i < items.length; i++) {
      var item = items[i];
      if (isDirectorsPage) {
        html += '<button class="legend-item legend-btn" data-tier="' + item.tier + '">' +
