(function () {
  ...
  function renderHomePage() {
    ...
    html += "</ul>";
    html += '<p class="updates-link"><a href="' + basePath + '/updates" data-page="updates"><em>' + escapeHtml(labels.updates) + '</em></a></p>';
  ...
  function renderAboutPage() {
    ...
    // Injection directe sans escapeHtml : langData.about contient du HTML intentionnel
    // (balises <em> pour les citations, guillemets stylistiques, etc.)
    html += '<div class="intro justified"><p>' + langData.about + "</p></div>";
    document.getElementById("main-content").innerHTML = html;
  }

  /**
   * Affiche la page "Dernières mises à jour".
   */
  function renderUpdatesPage() {
    var labels = i18n[currentLang];
    document.getElementById("mainNav").innerHTML = renderNav("updates");
    document.title = labels.updatesTitle + " — Mathieu Muzard's 1000 Essential Films";
    var html = '<h2 class="page-title">' + escapeHtml(labels.updatesTitle) + "</h2>";
    html += '<div class="intro justified"><p>' + (currentLang === "fr"
      ? "Version 1.0 — 28 mai 2026<br><br>Refonte complète de l'architecture (migration vers <code>data.json</code> + <code>app.js</code>), navigation par vraies URLs, menu hamburger mobile, filtre par tier, mode sombre, mosaïque de photos, interface bilingue FR/EN."
      : "Version 1.0 — May 28, 2026<br><br>Full architecture overhaul (migration to <code>data.json</code> + <code>app.js</code>), real URL routing, mobile hamburger menu, tier filter, dark mode, photo mosaic, bilingual FR/EN interface."
    ) + "</p></div>";
    document.getElementById("main-content").innerHTML = html;
  }
  ...
  function renderDirectorsPage() {
    ...
    for (var a = 0; a < alphabet.length; a++) {
      ...
      if (presentLetters[letter]) {
        html += '<a href="#dir-' + letter + '" class="alpha-letter">' + letter + '</a>';
      } else {
    ...
    for (var i = 0; i < sortedDirectors.length; i++) {
      ...
      // Ancre de lettre sur le premier réalisateur de chaque lettre
