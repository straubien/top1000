(function () {
    ...
    decades.forEach(function (decade) {
      ...
      years.forEach(function (year) {
        var films = yearsMap[year].map(function (r) {
          var credit = r["credit_decennie_" + lang];
          var film = { title: r["titre_" + lang] + " (" + credit + ")", tier: r.tier };
  ...
  function renderDirectorsPage() {
    ...
    for (var i = 0; i < sortedDirectors.length; i++) {
      ...
      if (director.firstname) {
        ...
        wikiFullName = director.firstname + " " + director.name;
        ...
        wikiFullName = parts.slice(1).join(" ") + " " + parts[0];
      } else {
        dirDisplayName = escapeHtml(director.name);
        wikiFullName = director.name;
      }
      if (WIKI_DISABLED_NAMES.indexOf(director.name) === -1) {
        var wikiOverride = WIKI_OVERRIDES[director.name];
        var wikiAttrs = 'data-wiki-name="' + escapeHtml(wikiFullName) + '"';
        if (wikiOverride) {
          wikiAttrs += ' data-wiki-override-title="' + escapeHtml(wikiOverride.title) +
            '" data-wiki-override-lang="' + escapeHtml(wikiOverride.lang) + '"';
        }
        dirDisplayName = '<span class="director-name-trigger" tabindex="0" role="button" ' +
          'aria-haspopup="true" ' + wikiAttrs + '">' +
          dirDisplayName + "</span>";
      }
  ...
  // Correspondances exactes pour les entrées ambiguës (collectifs, duos,
  // noms sans prénom) où la recherche automatique risquerait de se tromper.
  var WIKI_OVERRIDES = {
    "Engström/Theuring": { lang: "fr", title: "Ingemo Engström" },
    "Groupes Medvedkine": { lang: "fr", title: "Groupe Medvedkine" },
    "Léger/Murphy": { lang: "fr", title: "Fernand Léger" },
    "opér. Lumière": { lang: "fr", title: "Liste des films de la Société Lumière par pays de tournage" },
    "Straub/Huillet": { lang: "fr", title: "Jean-Marie Straub et Danièle Huillet" }
  };

  // Entrées sans page Wikipedia connue : pas d'info-bulle pour éviter
  // d'afficher un résultat potentiellement erroné.
  var WIKI_DISABLED_NAMES = ["Levitt/Loeb/Agee", "Winter/Edström"];
  ...
  function renderWikiTooltipContent(name, info) {
    var notFoundLabel = currentLang === "fr" ? "Aucune information trouvée." : "No information found.";
    var linkLabel = currentLang === "fr" ? "Voir sur Wikipedia →" : "See on Wikipedia →";
    if (!info) {
      return '<p class="wiki-tooltip-empty">' + escapeHtml(notFoundLabel) + "</p>";
    }
  ...
  function showWikiTooltipFor(triggerEl) {
    ...
    if (wikiTooltipCache.hasOwnProperty(cacheKey)) {
      if (wikiTooltipCurrentTrigger === triggerEl) {
        tooltip.innerHTML = renderWikiTooltipContent(name, wikiTooltipCache[cacheKey]);
        positionWikiTooltip(triggerEl);
      }
      return;
    }
