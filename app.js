(function () {
  ...
  // Cache Wikipedia en mémoire (clé : "lang:query")
  var wikiCache = {};

  // Élément tooltip unique dans le DOM
  var tooltipEl = null;
  ...
  function renderFilmItem(film) {
    ...
    // Séparer "Titre principal (parenthèse)" pour permettre deux zones de survol
    var parenMatch = film.title.match(/^(.+?)\s*(\([^)]+\))\s*$/);
    var titleHtml;
    if (parenMatch) {
      titleHtml =
        '<span class="film-title-main">' + escapeHtml(parenMatch[1]) + '</span>' +
        ' <span class="film-title-paren">' + escapeHtml(parenMatch[2]) + '</span>';
    } else {
      titleHtml = '<span class="film-title-main">' + escapeHtml(film.title) + '</span>';
    }
    return '<li class="' + film.tier + '">' + titleHtml + note + "</li>";
  }
  ...
  function renderCurrentPage() {
    ...
    transitionTimeoutId = setTimeout(function () {
      ...
      // Attacher les tooltips Wikipedia après chaque rendu
      initTooltip();
  ...
  function fetchWikipedia(query, callback) {
    var lang = currentLang === "en" ? "en" : "fr";
    var cacheKey = lang + ":" + query;
    if (wikiCache.hasOwnProperty(cacheKey)) {
      callback(wikiCache[cacheKey]);
      return;
    }
    ...
    // JSONP : contourne CORS et les extensions — Wikipedia charge comme un <script>
    var cbName = "wikiCb" + Date.now() + Math.floor(Math.random() * 9999);
    ...
    window[cbName] = function (json) {
      ...
      if (!pages) { finish(null); return; }
      var page = pages[Object.keys(pages)[0]];
      if (!page || page.hasOwnProperty("missing") || !page.extract) { finish(null); return; }
      finish({
        title:     page.title,
    ...
    script.src = "https://" + lang + ".wikipedia.org/w/api.php" +
      "?action=query" +
      "&generator=search" +
  ...
  function positionTooltip(el, cx, cy) {
    ...
    var left = cx + margin;
    var top  = cy + margin;
    if (left + w > vw - margin) { left = cx - w - margin; }
    if (top  + h > vh - margin) { top  = cy - h - margin; }
  ...
  function buildTooltipHtml(wikiData) {
    var html = "";
    if (wikiData.thumbnail) {
      html += '<img class="wiki-tooltip__img" src="' + escapeHtml(wikiData.thumbnail) + '" alt="">';
    }
    html += '<div class="wiki-tooltip__body">';
    html += '<strong class="wiki-tooltip__title">' + escapeHtml(wikiData.title) + '</strong>';
    var extract = wikiData.extract.length > 220
      ? wikiData.extract.slice(0, 220) + "…"
      : wikiData.extract;
    html += '<p class="wiki-tooltip__extract">' + escapeHtml(extract) + '</p>';
    html += '</div>';
    return html;
  }

  function attachTooltip(el, getQuery) {
    el.addEventListener("mouseenter", function (e) {
      var query = getQuery(el);
      if (!query) return;
      var tip = getOrCreateTooltipEl();
      tip.innerHTML = '<span class="wiki-tooltip__loading">…</span>';
      tip.classList.add("wiki-tooltip--visible");
      positionTooltip(tip, e.clientX, e.clientY);
      fetchWikipedia(query, function (wikiData) {
        if (!tip.classList.contains("wiki-tooltip--visible")) return;
        if (!wikiData) {
          tip.classList.remove("wiki-tooltip--visible");
          return;
        }
        tip.innerHTML = buildTooltipHtml(wikiData);
        positionTooltip(tip, e.clientX, e.clientY);
      });
    });

    el.addEventListener("mousemove", function (e) {
      if (tooltipEl && tooltipEl.classList.contains("wiki-tooltip--visible")) {
  ...
  // Retourne true si le contenu entre parenthèses est une année ou une abréviation technique
  function isYearOrAnnotation(paren) {
    var inner = paren.replace(/^\(|\)$/g, "").trim();
    return /^\d{4}$/.test(inner) || /^opér\./i.test(inner);
  }

  function initTooltip() {
    // Titre principal du film (span.film-title-main) — " film" oriente la recherche
    document.querySelectorAll(".film-list li .film-title-main").forEach(function (span) {
      attachTooltip(span, function (el) {
        var title = el.textContent.trim();
        return title ? title + " film" : null;
      });
    });

    // Partie entre parenthèses (span.film-title-paren) : cinéaste ou ignorée si année
    document.querySelectorAll(".film-list li .film-title-paren").forEach(function (span) {
      var text = span.textContent.trim();
      if (isYearOrAnnotation(text)) return; // année → pas de tooltip
      attachTooltip(span, function (el) {
        // Extraire le nom brut sans les parenthèses
        return el.textContent.replace(/^\(|\)$/g, "").trim() || null;
      });
    });

    // Noms de réalisateurs : "Prénom Nom" reconstruit
    document.querySelectorAll(".director-heading").forEach(function (h3) {
      attachTooltip(h3, function (el) {
        var lastname = "";
        el.childNodes.forEach(function (node) {
          if (node.nodeType === Node.TEXT_NODE) { lastname += node.textContent; }
        });
        lastname = lastname.trim();
        var firstnameSpan = el.querySelector(".director-firstname");
        if (firstnameSpan) {
          var firstname = firstnameSpan.textContent.replace(/^,\s*/, "").trim();
          return firstname ? firstname + " " + lastname : lastname;
        }
        // Format "Nom, Prénom" sans span
