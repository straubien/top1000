(function () {
  ...
  function fetchWikipedia(query, callback) {
    ...
    // Timeout 6 s pour ne pas rester bloqué sur "…"
    var timer = setTimeout(function () { finish(null); }, 6000);

    // API MediaWiki avec origin=* : CORS explicitement autorisé côté serveur
    ...
    fetch(url)
      .then(function (r) {
        if (!r.ok) { clearTimeout(timer); finish(null); return null; }
        return r.json();
      })
      .then(function (json) {
        if (!json) return;
        ...
        if (!page || page.missing === "" || !page.extract) { finish(null); return; }
        finish({
          title:     page.title,
