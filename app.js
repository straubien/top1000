(function () {
  ...
  var i18n = {
    fr: {
      ...
      statsDecade: "films",
      statsDirectors: "Réalisateurs les plus représentés",
      updateAdd: "Ajout",
      updateRemove: "Suppression",
      updateTier: "Changement de tier",
    ...
    en: {
      ...
      statsDecade: "films",
      statsDirectors: "Most represented directors",
      updateAdd: "Added",
      updateRemove: "Removed",
      updateTier: "Tier change",
  ...
  // Données des mises à jour chargées depuis updates.json
  var updatesData = [];
  ...
  function renderTierFilter() {
    ...
    for (var i = 0; i < tierKeys.length; i++) {
      var isActive = (tierKeys[i] === activeTier) ? ' class="active"' : "";
      html +=
        "<button" + isActive +
  ...
  function bindTierFilter() {
    ...
    filmItems.forEach(function (li) {
      li.style.display =
        activeTier === "all" || li.classList.contains(activeTier) ? "" : "none";
    });
  ...
  /**
   * Génère le HTML du bloc statistiques pour la page d'accueil.
   */
  function renderStats() {
    ...
    html += '<p class="home-section-title">' + escapeHtml(labels.statsTitle) + '</p>';
    html += '<ul class="stats-list">';
    for (var i = 0; i < langData.decades.length; i++) {
      var decade = langData.decades[i];
      var count  = countFilmsInDecade(decade);
      html += '<li>' + escapeHtml(decade.label) + ' — <span>' + count + ' ' + escapeHtml(labels.statsDecade) + '</span></li>';
    }
    html += '</ul>';

    // Réalisateurs les plus représentés (top 10)
    var dirs = langData.directors.slice();
    dirs.sort(function(a, b) { return b.films.length - a.films.length; });
    var topDirs = dirs.slice(0, 10);

    html += '<p class="home-section-title" style="margin-top:1.2rem">' + escapeHtml(labels.statsDirectors) + '</p>';
    html += '<ul class="stats-list">';
    for (var j = 0; j < topDirs.length; j++) {
      var dir = topDirs[j];
      var filmWord = dir.films.length === 1 ? labels.filmSingular : labels.films;
      html += '<li>' + escapeHtml(dir.name) + ' — <span>' + dir.films.length + ' ' + escapeHtml(filmWord) + '</span></li>';
    }
    html += '</ul>';
    html += '</div>';
    return html;
  ...
  /**
   * Génère le HTML du bloc mises à jour pour la page d'accueil.
   */
  function renderUpdates() {
    ...
    var html = '<div class="home-section">';
    html += '<p class="home-section-title">' + escapeHtml(labels.updates) + '</p>';

    for (var i = 0; i < updatesData.length; i++) {
      var group = updatesData[i];
      if (!group.entries || group.entries.length === 0) continue;

      html += '<p class="updates-date">' + escapeHtml(group.date) + '</p>';
      ...
      for (var j = 0; j < group.entries.length; j++) {
        ...
        if (entry.type === 'add') {
          cssClass = 'update-add';
          prefix   = escapeHtml(labels.updateAdd) + ' — ';
        } else if (entry.type === 'remove') {
          cssClass = 'update-remove';
          prefix   = escapeHtml(labels.updateRemove) + ' — ';
        } else if (entry.type === 'tier') {
          cssClass = 'update-tier';
          prefix   = escapeHtml(labels.updateTier) + ' — ';
        }

        var titleStr = escapeHtml(entry.title) + ' (' + escapeHtml(entry.director) + ', ' + entry.year + ')';

        if (entry.type === 'tier') {
          titleStr += ' — ' + escapeHtml(entry.from) + ' → ' + escapeHtml(entry.to);
        } else if (entry.type === 'add' && entry.tier) {
          titleStr += ' — ' + escapeHtml(entry.tier);
        }

        html += '<li class="' + cssClass + '">' + prefix + titleStr + '</li>';
      }
  ...
  function renderHomePage() {
    ...
    var html = '<div class="intro justified"><p>' + escapeHtml(langData.intro) + "</p></div>";
  ...
  function renderDirectorsPage() {
    ...
    var sortedDirectors = langData.directors.slice();
    var currentSort = renderDirectorsPage._currentSort || "alpha";

    if (currentSort === "count") {
      sortedDirectors.sort(function(a, b) { return b.films.length - a.films.length; });
    } else if (currentSort === "tier") {
      var tierOrder = { "top-100": 0, "top-200": 1, "top-300": 2, "top-rest": 3 };
      sortedDirectors.sort(function(a, b) {
        var bestA = a.films.reduce(function(best, f) {
          return tierOrder[f.tier] < tierOrder[best] ? f.tier : best;
        }, "top-rest");
        var bestB = b.films.reduce(function(best, f) {
          return tierOrder[f.tier] < tierOrder[best] ? f.tier : best;
        }, "top-rest");
        return tierOrder[bestA] - tierOrder[bestB];
      });
    }
    ...
    // Boutons de tri
    html += '<div class="sort-filter">';
    html += '<button' + (currentSort === "alpha"  ? ' class="active"' : "") + ' data-sort="alpha">' + (currentLang === "fr" ? "A–Z" : "A–Z") + '</button>';
    html += '<button' + (currentSort === "count"  ? ' class="active"' : "") + ' data-sort="count">' + (currentLang === "fr" ? "Nombre de films" : "Film count") + '</button>';
    html += '<button' + (currentSort === "tier"   ? ' class="active"' : "") + ' data-sort="tier">' + (currentLang === "fr" ? "Tier le plus élevé" : "Highest tier") + '</button>';
    html += '</div>';
    ...
    // Binding des boutons de tri
    document.querySelectorAll(".sort-filter button").forEach(function(btn) {
      btn.addEventListener("click", function() {
        renderDirectorsPage._currentSort = this.dataset.sort;
        renderDirectorsPage();
      });
    });
  }
  ...
  function initMosaicBanner() {
    ...
    // 4. Détecter dynamiquement le préfixe de chemin depuis la balise <script>
    var prefix = "";
    var scripts = document.getElementsByTagName("script");
    for (var s = 0; s < scripts.length; s++) {
      var src = scripts[s].getAttribute("src") || "";
      if (src.indexOf("app.js") !== -1) {
        // Extraire uniquement le chemin, sans le nom du fichier ni les paramètres de cache-busting
        prefix = src.split("app.js")[0];
        break;
      }
    }
    ...
    function loadOne() {
      ...
      img.onload = function () {
        loaded.push(photoIndex);
        var updatedMemory = memory.concat(loaded);
        if (updatedMemory.length > 30) {
          updatedMemory = updatedMemory.slice(updatedMemory.length - 30);
        }
        try {
          sessionStorage.setItem("bannerMemory", JSON.stringify(updatedMemory));
        } catch (e) {}
      };
    ...
    // Lancer les 6 chargements simultanément
    for (var t = 0; t < 6; t++) loadOne();
    ...
    setTimeout(function () {
      for (var p = 0; p < 12; p++) {
        ...
        if (nextNum.length < 2) nextNum = "0" + nextNum;
  ...
  function init() {
    ...
    // Correction #1 : ajout d'un .catch() sur le fetch de data.json
    // Charger data.json et updates.json en parallèle
    Promise.all([
      fetch(basePath + "/data.json").then(function(r) { return r.json(); }),
      fetch(basePath + "/updates.json").then(function(r) { return r.json(); }).catch(function() { return []; })
    ])
      .then(function (results) {
        data        = results[0];
        updatesData = results[1];
