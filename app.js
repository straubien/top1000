...
(function () {
  ...
  function renderNav(activeKey) {
    var ui = UI[LANG];
    var links = [
      { key: 'home', label: ui.home },
      { key: 'about', label: ui.about },
      { key: 'directors', label: ui.directors }
    ];
    for (var i = 0; i < DECADE_KEYS.length; i++) {
      var k = DECADE_KEYS[i];
      links.push({ key: k, label: getDecadeLabel(k) });
    }
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      html += '<a href="' + BASE + '/' + l.key + '" data-page="' + l.key + '"' + style + '>' + esc(l.label) + '</a>';
    }
  ...
  function renderHomePage() {
    ...
    for (var i = 0; i < langData.decades.length; i++) {
      ...
      var c = countFilms(d);
      html += '<li><a href="' + BASE + '/' + d.key + '" data-page="' + d.key + '">' + esc(d.label) + '</a> <span class="film-count">(' + c + ' ' + ui.films + ')</span></li>';
    }
    html += '</ul>';
  ...
  function renderDecadePage(key) {
    ...
    if (idx > 0) {
      ...
      html += '<a href="' + BASE + '/' + prevKey + '" data-page="' + prevKey + '">&larr; ' + esc(getDecadeLabel(prevKey)) + '</a>';
    } else {
      html += '<span></span>';
    }
    if (idx < DECADE_KEYS.length - 1) {
      var nextKey = DECADE_KEYS[idx + 1];
      html += '<a href="' + BASE + '/' + nextKey + '" data-page="' + nextKey + '">' + esc(getDecadeLabel(nextKey)) + ' &rarr;</a>';
    } else {
      html += '<span></span>';
    }
    html += '</div>';
  ...
  function route() {
    ...
    var prevPage = route._currentPage || 'home';
    route._scrollPositions = route._scrollPositions || {};
    route._scrollPositions[prevPage] = window.scrollY;
    route._currentPage = page;
    ...
    setTimeout(function () {
      if (page === 'home' || page === '') {
        ...
        renderDirectorsPage();
      } else if (DECADE_KEYS.indexOf(page) !== -1) {
      ...
      var savedScroll = route._scrollPositions[page] || 0;
  ...
  function setupBanner() {
    ...
    function addImage() {
      ...
      img.src = BASE + '/images/photo (' + num + ').jpg';
      img.alt = '';
      img.onload = function () {
        used.push(n);
