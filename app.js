...
(function () {
  ...
  var UI = {
    fr: {
      ...
      decadePrev: '←',
      decadeNext: '→',
      metaDesc: 'Top 1000 films essentiels de Mathieu Muzard, classés chronologiquement de 1895 à 2023.',
    ...
    en: {
      ...
      decadePrev: '←',
      decadeNext: '→',
      metaDesc: "Mathieu Muzard's 1000 essential films, listed chronologically from 1895 to 2023.",
  ...
  function renderTierFilter() {
    var ui = UI[LANG];
    return '<div class="tier-filter">' +
      '<button class="active" data-tier="all">' + esc(ui.all) + '</button>' +
      '<button data-tier="top-100">Top 100</button>' +
      '<button data-tier="top-200">Top 200</button>' +
      '<button data-tier="top-300">Top 300</button>' +
      '<button data-tier="top-rest">Top 1000</button>' +
      '</div>';
  }
      ...
      btn.addEventListener('click', function () {
        ...
        items.forEach(function (li) {
          if (tier === 'all') {
            ...
            li.style.display = li.classList.contains(tier) ? '' : 'none';
          }
        });
      });
    });
  ...
  function renderHomePage() {
    var ui = UI[LANG];
    var langData = DATA[LANG];
    document.querySelector('nav').innerHTML = renderNav('home');
  ...
  function renderAboutPage() {
    var ui = UI[LANG];
    var langData = DATA[LANG];
    document.querySelector('nav').innerHTML = renderNav('about');
  ...
  function renderDirectorsPage() {
    var ui = UI[LANG];
    var langData = DATA[LANG];
    document.querySelector('nav').innerHTML = renderNav('directors');
    ...
    for (var i = 0; i < langData.directors.length; i++) {
      ...
      html += '<h3 class="director-heading">' + esc(dir.name) + '</h3>';
  ...
  function renderDecadePage(key) {
    ...
    document.querySelector('nav').innerHTML = renderNav(key);
    ...
    if (idx > 0) {
      ...
      html += '<a href="#' + prevKey + '">' + ui.decadePrev + ' ' + esc(getDecadeLabel(prevKey)) + '</a>';
    } else {
      html += '<span></span>';
    }
    if (idx < DECADE_KEYS.length - 1) {
      var nextKey = DECADE_KEYS[idx + 1];
      html += '<a href="#' + nextKey + '">' + esc(getDecadeLabel(nextKey)) + ' ' + ui.decadeNext + '</a>';
    } else {
      html += '<span></span>';
    }
    html += '</div>';
