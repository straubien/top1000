(function () {
  ...
  /**
   * Affiche la page "Dernières mises à jour" en lisant CHANGELOG.md.
   */
  function renderUpdatesPage() {
    ...
    html += '<div id="changelog-content" class="intro"><p>…</p></div>';
    document.getElementById("main-content").innerHTML = html;

    // Charger et afficher le CHANGELOG.md
    fetch(basePath + "/CHANGELOG.md")
      .then(function(response) { return response.text(); })
      .then(function(text) {
        // Convertir le markdown en HTML minimal
        var lines = text.split("\n");
        var result = "";
        for (var i = 0; i < lines.length; i++) {
          var line = lines[i];
          if (line.startsWith("## ")) {
            result += '<h3 class="updates-version">' + escapeHtml(line.replace("## ", "")) + "</h3>";
          } else if (line.startsWith("### ")) {
            result += '<h4 class="updates-section">' + escapeHtml(line.replace("### ", "")) + "</h4>";
          } else if (line.startsWith("- ")) {
            result += '<p class="updates-item">' + escapeHtml(line.replace("- ", "")) + "</p>";
          } else if (line.startsWith("# ")) {
      ...
      .catch(function() {
        var container = document.getElementById("changelog-content");
        if (container) container.innerHTML = "<p>Impossible de charger le changelog.</p>";
      });
  }
  ...
  function renderDirectorsPage() {
    ...
    for (var a = 0; a < alphabet.length; a++) {
      ...
      if (presentLetters[letter]) {
        html += '<button class="alpha-letter" data-letter="' + letter + '">' + letter + '</button>';
      } else {
